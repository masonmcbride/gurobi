...
try: 
    ...
    x = np.array([[0.20,0.60,0.20]]) # [0 1 0] plays paper 100% of the time

    F = np.ones((num_actions, 1)) # vector of all ones
    f = np.array([1])
    ...
    # create variables
    y = m.addMVar(num_actions, ub=1.0)

    # create objective
    m.setObjective(x @ -A @ y, GRB.MAXIMIZE)

    # add constraints
    m.addConstr(F.T @ y == f)
    m.addConstr(y >= 0) 
    #m.addConstr(y[1] <= 0.50)

    m.optimize()

    print(f"\n\t\t\t     R   P   S")
    print(f"player 1 playing strategy: {x}")
    print(f"player 2 best responsea: {y.X}")
